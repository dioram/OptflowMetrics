    ...
    {                                                                                               \
        ...
        {                                                                                           \
            ...
            case 1:                                                                                 \
                ...
                CV_Error(cv::Error::StsBadFunc, errorLog.str());                                        \
                break;                                                                              \
            case 2:                                                                                 \
                errorLog << #nvOFAPI << " returned error " << (unsigned int)errorCode;              \
                errorLog << ":NV_OF_ERR_UNSUPPORTED_DEVICE";                                        \
                CV_Error(cv::Error::StsBadArg, errorLog.str());                                         \
                break;                                                                              \
            case 3:                                                                                 \
                errorLog << #nvOFAPI << " returned error " << (unsigned int)errorCode;              \
            ...
            case 4:                                                                                 \
                ...
                CV_Error(cv::Error::StsNullPtr, errorLog.str());                                        \
                break;                                                                              \
            case 5:                                                                                 \
                errorLog << #nvOFAPI << " returned error " << (unsigned int)errorCode;              \
            ...
            case 6:                                                                                 \
                ...
                CV_Error(cv::Error::BadCallBack, errorLog.str());                                       \
                break;                                                                              \
            case 7:                                                                                 \
                errorLog << #nvOFAPI << " returned error " << (unsigned int)errorCode;              \
                errorLog << ":NV_OF_ERR_INVALID_VERSION";                                           \
                CV_Error(cv::Error::StsError, errorLog.str());                                          \
                break;                                                                              \
            case 8:                                                                                 \
                errorLog << #nvOFAPI << " returned error " << (unsigned int)errorCode;              \
                errorLog << ":NV_OF_ERR_OUT_OF_MEMORY";                                             \
                CV_Error(cv::Error::StsNoMem, errorLog.str());                                          \
                break;                                                                              \
            case 9:                                                                                 \
                errorLog << #nvOFAPI << " returned error " << (unsigned int)errorCode;              \
            ...
            case 11:                                                                                \
                ...
                CV_Error(cv::Error::StsInternal, errorLog.str());                                       \
                break;                                                                              \
            default:                                                                                \
                break;                                                                              \
            }                                                                                       \
...
namespace
{
    ...
    private:
            ...
            m_isFailed(false)
        {
            ...
            {
                m_isFailed = true;
                CV_Error(cv::Error::StsBadFunc, "Cannot find Cuda library.");
            }
            ...
            {
                m_isFailed = true;
                CV_Error(cv::Error::StsBadFunc, "Cannot find NvOF library.");
            }
    ...
    public:
        static LoadNvidiaModules& Init()
        {
            ...
            if (LoadLibraryObj.m_isFailed)
                CV_Error(cv::Error::StsError, "Can't initialize LoadNvidiaModules Class Object");
    ...
    class NvidiaOpticalFlowImpl : public NvidiaOpticalFlow_2_0
    {
    private:
        int m_width;
        int m_height;
        ...
        int m_gpuId;
        cv::cuda::Stream m_inputStream;
        cv::cuda::Stream m_outputStream;
    ...
    public:
        NvidiaOpticalFlowImpl(int width, int height, NV_OF_PERF_LEVEL perfPreset, bool bEnableTemporalHints,
            bool bEnableExternalHints, bool bEnableCostBuffer, int gpuId, cv::cuda::Stream inputStream, cv::cuda::Stream outputStream);

        virtual void calc(cv::InputArray inputImage, cv::InputArray referenceImage,
            cv::InputOutputArray flow, cv::cuda::Stream& stream = cv::cuda::Stream::Null(),
            cv::InputArray hint = cv::noArray(), cv::OutputArray cost = cv::noArray());


        virtual void collectGarbage();

        virtual void upSampler(cv::InputArray flow, int width, int height,
            int gridSize, cv::InputOutputArray upsampledFlow);
        ...
        m_gridSize(NV_OF_OUTPUT_VECTOR_GRID_SIZE_4)
    {
        ...
        {
            CV_Error(cv::Error::StsBadArg, "Invalid GPU Ordinal");
        }

        cudaSafeCall(cudaSetDevice(m_gpuId));
        cudaSafeCall(cudaFree(m_cuContext));

        cudaSafeCall((cudaError_t)LoadNvidiaModulesObj.GetCudaLibraryFunctionPtr()(&m_cuContext));

        if (m_gridSize != NV_OF_OUTPUT_VECTOR_GRID_SIZE_4)
        {
            CV_Error(cv::Error::StsBadArg, "Unsupported grid size");
        }
    ...
    void NvidiaOpticalFlowImpl::calc(cv::InputArray _frame0, cv::InputArray _frame1, cv::InputOutputArray _flow,
        cv::cuda::Stream& stream, cv::InputArray hint, cv::OutputArray cost)
    {
        ...
        cv::cuda::GpuMat flowXYGpuMat(cv::Size((m_width + m_gridSize - 1) / m_gridSize,
        ...
        {
            //Get Mats from InputArrays
            cv::Mat __frame1 = _frame1.getMat();
            frame1GpuMat.upload(__frame1, m_inputStream);
        }
        else if (_frame1.isGpuMat())
        {
            //Get GpuMats from InputArrays
            cv::cuda::GpuMat __frame1 = _frame1.getGpuMat();
        ...
        if (_flow.isMat())
            ...
            CV_Error(cv::Error::StsBadArg, "Incorrect flow buffer passed. Pass Mat or GpuMat");

        if (m_enableCostBuffer)
        {
            cv::cuda::GpuMat costGpuMat(cv::Size((m_width + m_gridSize - 1) / m_gridSize,
            ...
            if (cost.isMat())
                ...
                CV_Error(cv::Error::StsBadArg, "Incorrect cost buffer passed. Pass Mat or GpuMat");
        }
    ...
    void NvidiaOpticalFlowImpl::upSampler(cv::InputArray _flow, int width, int height,
        int gridSize, cv::InputOutputArray upsampledFlow)
    {
        cv::Mat flow;
        if (_flow.isMat())
        {
            cv::Mat __flow = _flow.getMat();
            __flow.copyTo(flow);
        }
        else if (_flow.isGpuMat())
        {
            cv::cuda::GpuMat __flow = _flow.getGpuMat();
        ...
        cv::Mat output(cv::Size(width, height), CV_32FC2, flowVectors.get());
...
cv::Ptr<NvidiaOpticalFlow_2_0> NvidiaOpticalFlow_2_0::create(
    int width, int height, NvidiaOpticalFlow_2_0::NVIDIA_OF_PERF_LEVEL perfPreset,
    bool bEnableTemporalHints, bool bEnableExternalHints,
    bool bEnableCostBuffer, int gpuId,
    cv::cuda::Stream& inputStream, cv::cuda::Stream& outputStream)
{
    return cv::makePtr<NvidiaOpticalFlowImpl>(
        width,
        height,
